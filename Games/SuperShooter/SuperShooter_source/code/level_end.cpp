...
void LevelEnd::Init(uInt32 data)
{
   lScore = 0;
   ...
   music.LoadMidi("sound\\end.mid", 0);
...
void LevelEnd::Exit(uInt32 data)
{
   music.Close();
}
...
void LevelEnd::Draw()
{
   if( !DisplayLoading )
   {
      ...
      for(int i = 1; i < 4; i++)
      {
         sprintf(temp_str, "  %d        %d", i, sds->lScore[i-1]);
         info.SetText(temp_str);
         info.SetPos(x, y);
         info.Draw();
         y -= 15;
      }
      ...
      sprintf(temp_str, "Total      %d", gScore);
      info.SetText(temp_str);
      info.SetPos(x, y);
      info.Draw();
      ...
      info.SetText("Press ENTER to try again.");
      info.SetPos(x, y);
      info.Draw();
...
void LevelEnd::StopMusic()
{
   music.Stop();
}

void LevelEnd::KeyPressed(bool down, bool skey, int key, int x, int y)
{
   if( down )
   {
      if( key == 13)
...
void LevelEnd::MouseClick(bool down, int button, int x, int y)
{

}


void LevelEnd::MouseMoved(int x, int y)
{
   
}

#define TITLE_POINT(P1, P2) glVertex2i(x + P1*seg_length, y + P2*seg_length);
void LevelEnd::DrawTitle()
{
...
void LevelEnd::DrawShootingStars()
{
...
void LevelEnd::UpdateStars(uInt8 bkgn, uInt8 ln)
{
   if(bkgn == 0 || bkgn == 1)
   {
      x = MAX_WIDTH;
