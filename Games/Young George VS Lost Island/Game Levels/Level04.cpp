...
void Start_L4(void)
{
	...
	level->lSpriteA[0].cel = game->var[2];
...
void Mid_L4(void)
{
	...
	if(global->sound.midi.MidiStat == IDM_NULL)
		...
		{
		if(strcmp(global->sound.midi.mf.file, IDM_LEVEL04) != 0)
			{
			global->sound.midi.mf.sec = 0;
			strcpy(global->sound.midi.mf.file, IDM_LEVEL04);
			MidiProc(IDM_PLAY);
...
if(id == IDTALK)
	{
	if(strcmp(item1, level->lZoneA[4].zoneName) == 0)
		{
			LoadString(LevelhInst, IDST_MOON_L4, (LPTSTR)str, sizeof(str));
//			LoadString(LevelhInst, IDST_MOON_L4a, (LPTSTR)SelectStr[0], sizeof(SelectStr[0]));
		}
	else if(strcmp(item1, level->lZoneA[1].zoneName) == 0)
			LoadString(LevelhInst, IDST_SELF_L4, (LPTSTR)str, sizeof(str));
	else
		LoadString(LevelhInst, IDST_ERROR_L4, (LPTSTR)str, sizeof(str));
	}
...
//			LoadString(LevelhInst, IDST_T_SIGN1a, (LPTSTR)SelectStr[0], sizeof(SelectStr[0]));
	...
	if(strcmp(item1, level->lZoneA[2].zoneName) == 0)
		{
		if((level->lSpriteA[0].cel == 2) || (level->lSpriteA[0].cel == 5))
			LoadString(LevelhInst, IDST_OPEN_L4_2, (LPTSTR)str, sizeof(str));
		else
			LoadString(LevelhInst, IDST_OPEN_L4_1, (LPTSTR)str, sizeof(str));

		if(level->lSpriteA[0].cel == 0)
			level->lSpriteA[0].cel = 2;
		else if(level->lSpriteA[0].cel == 3)
			level->lSpriteA[0].cel = 5;
...
//			LoadString(LevelhInst, IDST_T_SIGN1a, (LPTSTR)SelectStr[0], sizeof(SelectStr[0]));
	...
	if(strcmp(item1, level->lZoneA[2].zoneName) == 0)
		{
		if((level->lSpriteA[0].cel == 0) || (level->lSpriteA[0].cel == 3))
			LoadString(LevelhInst, IDST_CLOSE_L4_2, (LPTSTR)str, sizeof(str));
		else
			LoadString(LevelhInst, IDST_CLOSE_L4_1, (LPTSTR)str, sizeof(str));
		
		if(level->lSpriteA[0].cel == 2)
			level->lSpriteA[0].cel = 0;
		else if(level->lSpriteA[0].cel == 5)
			level->lSpriteA[0].cel = 3;
		global->LevelVar.Redraw = TRUE;
//			LoadString(LevelhInst, IDST_T_SIGN1a, (LPTSTR)SelectStr[0], sizeof(SelectStr[0]));
		}
	else
...
else if(id == IDPUSHPULL)
	{
	if(strcmp(item1, level->lZoneA[2].zoneName) == 0)
		{
		if((level->lSpriteA[0].cel == 0) || 
			 (level->lSpriteA[0].cel == 1) ||
			 (level->lSpriteA[0].cel == 2))
			{
			LoadString(LevelhInst, IDST_PUSH_L4_1, (LPTSTR)str, sizeof(str));
			level->lSpriteA[0].cel += 3;
			}
		else if((level->lSpriteA[0].cel == 3) ||
						(level->lSpriteA[0].cel == 4) ||
						(level->lSpriteA[0].cel == 5))
			{
			LoadString(LevelhInst, IDST_PUSH_L4_2, (LPTSTR)str, sizeof(str));
			level->lSpriteA[0].cel -= 3;
			}
		global->LevelVar.Redraw = TRUE;
		}
	...
	LoadString(LevelhInst, IDST_ERROR_L4, (LPTSTR)str, sizeof(str));
game->var[2] = level->lSpriteA[0].cel;
strcpy(tempstr, str);
return tempstr;
}